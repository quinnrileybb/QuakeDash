...
# … after defining tabs = st.tabs([... "Hit Zones"]) …
    ...
    # --- 2) Apply filters ---
            df_hz = batter_data.copy().dropna(subset=["PlateLocSide","PlateLocHeight"])
            if sc or bc:
    ...
    # --- 3) Setup helpers ---
            swing_set = {"StrikeSwinging","InPlay","FoulBallFieldable","FoulBallNotFieldable","FoulBall"}
            woba_wts  = {'Out':0,'Walk':0.69,'HitByPitch':0.72,'Single':0.88,
                         'Double':1.247,'Triple':1.578,'HomeRun':2.031}
            total_pts = len(df_hz)

    # --- 4) Define 13 pockets (IDs 1–9 in-zone, 11–14 out) ---
            ...
            for r in range(3):
                for c in range(3):
                    regions.append({
                        "id": n,
                        "xmin": zx[c], "xmax": zx[c+1],
                        "ymin": zy[2-r], "ymax": zy[3-r]
                    })
                    n += 1
            regions += [
    ...
    # --- 5) Compute metric per pocket ---
            ...
            for reg in regions:
                ...
                if selected_metric == "Pitch%":
                    ...
                    s = sub["PitchCall"].isin(swing_set).sum()
                    v = s/len(sub)*100 if len(sub)>0 else np.nan
                elif selected_metric == "Whiff%":
                    mask_s = sub["PitchCall"].isin(swing_set)
                    mask_w = sub["PitchCall"]=="StrikeSwinging"
                    arr = np.where(mask_s, mask_w.astype(int), np.nan)
                    v = np.nanmean(arr)*100 if mask_s.any() else np.nan
                    ...
                    v = sub["ExitSpeed"].mean() if sub["ExitSpeed"].dropna().any() else np.nan
                elif selected_metric == "90th % EV":
                    e = sub["ExitSpeed"].dropna()
                    v = np.percentile(e,90) if len(e)>0 else np.nan
                else:  # Max EV
                    e = sub["ExitSpeed"].dropna()
                    v = e.max() if len(e)>0 else np.nan
                vals.append(v)
    ...
    # outline strike zone
            outline = Rectangle((-0.83,1.5),1.66,2.0,
                                fill=False,edgecolor="black",linewidth=2)
